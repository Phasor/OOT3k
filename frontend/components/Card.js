...
export default function Card({profile}) {
  return (
    <div className='flex flex-col items-center p-2 max-w-[350px]'>
        <div className="">
            <Image
                src={profile.image}
                alt={profile.name}
            ...
            <div className='flex justify-between mt-3'>
                <p className='text-xl sm:text-2xl font-poppins font-medium text-gray-800 mt-2'>{profile.name}</p>
                <div className='bg-blue-500 h-10 w-10 rounded-lg flex justify-center mt-1 items-center text-center hover:scale-105'>
            ...
            <p className='font-raleway font-semibold text-xl mt-4 text-gray-100'>{profile.role}</p>
            <p className='font-raleway text-gray-800 text-l wrap mt-2'>{profile.description}</p>
        </div>
    </div>
  )
}
